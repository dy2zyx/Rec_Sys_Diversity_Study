...
path_trainset = '../../Datasets/LibraryThing/trainset.dat'
path_testset = '../../Datasets/LibraryThing/testset.dat'
items_embedding_file = '../../Datasets/LibraryThing/embeddings/ICKG/itemURI_embedding.json'
mapping_file = '../../Datasets/LibraryThing/mappings/MappingLibrarything2DBpedia-1.3.tsv'
trainset_entity_embedding_file = '../../Datasets/LibraryThing/embeddings/ICKG+UPKG/dict_entity_embedding.pickle'
trainset_relation_embedding_file = '../../Datasets/LibraryThing/embeddings/ICKG+UPKG/dict_relation_embedding.pickle'
...
user_profil_ild_file = '../../Datasets/LibraryThing/data_dicts/user_profile_ILD_dict.pickle'
with open(user_profil_ild_file, 'rb') as user_ilds:
    user_profil_ild_dict = pickle.load(user_ilds)

# sim_dict_file = '../../Datasets/LibraryThing/data_dicts/item_semantic_distance_dict.pickle'
...
users_have_liked_items = set([user for (user, item, rating) in testset if rating > 7 and USER+str(user) in dict_entity_embedding.keys()])
test_set = [(user, item) for (user, item, rating) in testset if rating > 7 and USER+str(user) in dict_entity_embedding.keys()]
...
def predict_rating(user, item):
    ...
    for i in user_positive_profils[user]:
        ...
        sim_i_item = 1 - cosine(dict_uri_embedding[dict_iid_uri[str(i)]], dict_uri_embedding[dict_iid_uri[str(item)]])
        rating += sim_i_item
...
def recommend_topN(user):
    top_n = list()
    for un_rated_item in user_unrated_items[user]:
        predicted_rating = predict_rating(user, un_rated_item)
...
for user in user_unrated_items.keys():
    user_candidate_items = recommend_topN(user)
