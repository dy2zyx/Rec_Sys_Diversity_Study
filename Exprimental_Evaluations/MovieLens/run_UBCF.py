...
path_trainset = '../../Datasets/MovieLens/trainset.pickle'
path_testset = '../../Datasets/MovieLens/testset.pickle'
items_embedding_file = '../../Datasets/MovieLens/embeddings/ICKG/itemURI_embedding.json'
mapping_file = '../../Datasets/MovieLens/mappings/MappingMovieLens2DBpedia-1.2.tsv'
trainset_entity_embedding_file = '../../Datasets/MovieLens/embeddings/ICKG+UPKG/dict_entity_embedding.pickle'
trainset_relation_embedding_file = '../../Datasets/MovieLens/embeddings/ICKG+UPKG/dict_relation_embedding.pickle'
...
user_profil_ild_file = '../../Datasets/MovieLens/data_dicts/user_profile_ILD_dict.pickle'
with open(user_profil_ild_file, 'rb') as user_ilds:
    user_profil_ild_dict = pickle.load(user_ilds)

# sim_dict_file = '../../Datasets/MovieLens/data_dicts/item_semantic_distance_dict.pickle'
# with open(sim_dict_file, 'rb') as sim_matrix:
#     distance_dict = pickle.load(sim_matrix)
dict_uri_embedding = dict()
dict_iid_uri = dict()

"""read item embeddings file and charge the item_uri->embedding_vector dictionnary"""
with open(items_embedding_file, mode='r') as file:
    data = json.load(file)
    for item, embedding in data.items():
        dict_uri_embedding[item] = embedding

"""read mapping file and charge the item_id->item_uri dictionnary"""
with open(mapping_file, mode='r') as file:
    lines = file.read().splitlines()
    for mapping in lines:
        item_raw_id = mapping.split(sep="	")[0]
        item_dbpedia_uri = mapping.split(sep="	")[2]
        dict_iid_uri[item_raw_id] = item_dbpedia_uri
...
users_have_liked_items = set([user for (user, item, rating, timestamp, taste) in testset if taste == 1])
test_set = [(user, item) for (user, item, rating, timestamp, taste) in testset if taste == 1]
...
data = pd.DataFrame(trainset, columns=['userID', 'itemID', 'rating', 'timestamps', 'taste'])
data = data.drop(columns="taste")
...
ubcf = KNNWithZScore(sim_options={'user_based': True, 'name': 'pearson'})
ubcf.fit(train_set)
...
def recommend_topN(user, model):
    ...
    for un_rated_item in user_unrated_items[user]:
        ...
        normalised_r = np.divide(predicted_rating-1, 4)
...
for user in user_unrated_items.keys():
    user_candidate_items = recommend_topN(user, ubcf)
...
def intra_list_diversity(input_list):
    ...
    for i in range(n):
        for j in range(i+1, n):
            dissimilarity += cosine(dict_uri_embedding[dict_iid_uri[input_list[i]]],
                                    dict_uri_embedding[dict_iid_uri[input_list[j]]])
...
def f_obj(persDiv, ild_profil, candidate_item, list_rec, alpha, candidats_dissim_dict, sum_dissim, sum_accuracy):
    ...
    if n == 0:
        ...
        div = cosine(dict_uri_embedding[dict_iid_uri[str(last_added_item)]], dict_uri_embedding[dict_iid_uri[str(id_candidate_item)]])
...
def bg_optimise(M, n, alpha, persDiv=False):
    ...
    with open('results/UBCF/bg_alpha'+str(alpha)+'_persDiv_'+str(persDiv)+'.csv', mode='w') as file:
